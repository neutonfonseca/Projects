...
class yelpreviews_spider(scrapy.Spider):
	...
	def parse(self,response):
		...
		for url in pageurl:
			print(url)
			yield scrapy.Request(url, callback = self.parse_top, meta={'url':url})
	...
	def parse_top(self,response):
		...
		for i in range(int(pages)):
			restauranturl.append(url + '?start=' + str(20*i))
	...
	def parse_page(self,response):
		...
		for review in reviews: 
			...
			delivery = review.xpath('//*[@id="super-container"]/div/div/div[2]/div[2]/div[3]/ul/li/div/dl[2]/dd/text()').extract()
